...
def process_video(video: dict, openai_client: OpenAI) -> None:
    ...
    try:
        ...
        # 2. Опційне перекодування — менший файл надійніше проходить тунель
        serve_path = video_path
        if TRANSCODE_MAX_HEIGHT > 0:
            serve_path = os.path.join(WORK_DIR, f"{video_id}_small.mp4")
            downloader.transcode_smaller(video_path, serve_path,
                                          max_height=TRANSCODE_MAX_HEIGHT)

        # 3. Тунель -> HeyGen -> чекаємо, поки заберуть файл
        ...
        # 7. Фіксуємо стан (шлях відносно кореня репо — Workflow 2 його закомітить)
        ...
        _cleanup([video_path, serve_path if serve_path != video_path else None, thumb_ref_path])
